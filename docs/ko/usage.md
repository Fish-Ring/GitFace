...
`[G]`로 저장소별 원격 경로 매핑을 관리:

| 키 | 동작 |
|---|---|
| `[Enter]` | 선택한 프로바이더의 저장소 경로 편집 |
| `[Esc]` | 뒤로 |
| ↑ ↓ / 스크롤 | 탐색 |

각 프로바이더에는 구성된 저장소 경로(예: `Fish-Ring/GitFace`)가 표시됩니다. 신분 전환 시 이 경로를 사용하여 원격 URL을 다시 작성합니다. 구성은 저장소의 `.git/gitf.json`에 저장됩니다.
...
2. 유형 선택：
   ...
   - `d` → `docs` (문서)
   - `r` → `refactor` (리팩토링)
3. 설명 입력 (여러 줄 지원), `F2`로 제출
4. 자동 `git add .` → `git commit -m "type: desc"` → `git push`
5. 결과 확인, 아무 키나 눌러 돌아가기

### 태그 생성

메인 화면에서 `[T]`를 눌러 릴리스 태그 생성：

1. 버전 (예 `v1.0.0`)을 입력하고 `Enter`로 확인
2. 태그가 생성되어 푸시됩니다

### 브랜치 전환

메인 화면에서 `[B]`를 눌러 브랜치 전환：

1. 로컬 브랜치 목록이 표시됩니다 (현재 브랜치에 `*` 표시)
2. ↑↓ 또는 스크롤로 이동
3. `Enter`로 전환, `Esc`로 취소
4. 전환 후 자동으로 상태 새로고침
